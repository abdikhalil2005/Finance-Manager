...
export function Fuel() {
  ...
  const openModal = (fuel?: Fuel) => {
    if (fuel) {
      setEditingId(fuel.id);
      setFormData({
        month: String(fuel.month),
        year: String(fuel.year),
        amount: String(fuel.amount),
        date_paid: fuel.date_paid || '',
        notes: fuel.notes || '',
      });
    } else {
      setEditingId(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      await supabase.from('fuel').update(data).eq('id', editingId);
    } else {
      await supabase.from('fuel').insert([data]);
    }
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this fuel record?')) {
      await supabase.from('fuel').delete().eq('id', id);
      loadFuels();
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-slate-800">Fuel Expenses</h3>
        <button
          onClick={() => openModal()}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-slate-50 rounded-lg border border-slate-200">
          <p className="text-slate-600">No fuel expenses found. Add your first record to get started.</p>
        </div>
      ) : (
              ...
              {fuels.map((fuel) => (
                <tr key={fuel.id} className="border-b border-slate-100 hover:bg-slate-50">
                  <td className="py-3 px-4 font-medium text-slate-900">
                    {getMonthName(fuel.month)} {fuel.year}
                  </td>
                  <td className="py-3 px-4 text-right text-slate-700">
                    {formatCurrency(fuel.amount)}
                  </td>
                  <td className="py-3 px-4 text-center text-slate-700">
                    {fuel.date_paid ? new Date(fuel.date_paid).toLocaleDateString() : '-'}
                  </td>
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full">
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <h3 className="text-xl font-bold text-slate-900">
                {editingId ? 'Edit Fuel Expense' : 'Add Fuel Expense'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="e.g., Downtown - 150km, Uptown - 200km"
                />
              </div>
